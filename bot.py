...
def giverole_command(message):
    ...
    try:
        ...
        if role_name not in ROLES:
            bot.reply_to(message, f"❌ Роль {role_name} не существует. Доступны: {', '.join(ROLES.keys())}")
            return
        if not is_registered(target_id):
            bot.reply_to(message, f"❌ Пользователь {target_id} не зарегистрирован")
            return
...
def export_command(message):
    ...
    try:
        users = load_json(USERS_FILE)
        csv_data = "ID,Username,Имя,Монеты,Сообщения,Роли,Активная роль,Инвайты,Регистрация\n"
        for uid, data in users.items():
            ...
            active = data.get('active_roles', [''])[0]
            invites = len(data.get('invites', []))
            reg_date = data.get('registered_at', '')
            csv_data += f"{uid},{username},{first_name},{coins},{messages},{roles},{active},{invites},{reg_date}\n"
        with open('users_export.csv', 'w', encoding='utf-8') as f:
            f.write(csv_data)
        with open('users_export.csv', 'rb') as f:
            bot.send_document(message.chat.id, f, caption="📊 Экспорт пользователей")
...
{'' if user['coins'] >= price else '❌ Не хватает монет!' if user['coins'] < price else '✅ Можешь купить!'}
    ...
    # Админские кнопки
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("📊 Введи код промо для статистики:\nНапример: /promostats HELLO",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("🗑 Введи код промо для удаления:\nНапример: /delpromo HELLO",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("👤 Введи ID пользователя:\nНапример: /userinfo 123456789",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("💰 Введи ID и сумму:\nНапример: /addcoins 123456789 1000",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("🎭 Введи ID и роль:\nНапример: /giverole 123456789 Vip\nДоступны: " + ', '.join(ROLES.keys()),
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("⏰ Введи ID, роль и дни:\nНапример: /tempgive 123456789 Vip 7",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("🚫 Введи ID и опционально дни и причину:\nНапример: /ban 123456789 7 За спам",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("✅ Введи ID:\nНапример: /unban 123456789",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
        ...
        if not is_admin(uid):
            bot.answer_callback_query(call.id, "❌ Нет прав", show_alert=True)
            return
        bot.edit_message_text("📢 Введи текст рассылки:\nНапример: /mailing Всем привет!",
                             call.message.chat.id, call.message.message_id, reply_markup=get_admin_keyboard())
        bot.answer_callback_query(call.id)
