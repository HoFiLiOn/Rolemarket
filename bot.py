...
# ========== ТВОЙ АДМИН-АККАУНТ ==========
MASTER_IDS = [8388843828]  # только этот админ
...
def delete_custom_section(callback):
    ...
    data['main_buttons'] = [s['name'] for s in data['sections']]
    save_custom_sections(data)

def add_section_button(section_callback, button_text, button_type, button_value):
    data = get_custom_sections()
    for section in data['sections']:
        if section['callback'] == section_callback:
            if 'buttons' not in section:
                section['buttons'] = []
...
def get_main_keyboard():
    ...
    for button_name in custom_data.get('main_buttons', []):
        for section in custom_data.get('sections', []):
            if section['name'] == button_name:
                buttons.append(types.InlineKeyboardButton(button_name, callback_data=f"custom_{section['callback']}"))
...
def get_shop_keyboard():
    ...
    for role in PERMANENT_ROLES:
        markup.add(types.InlineKeyboardButton(f"{role}", callback_data=f"perm_{role}"))
        ...
        for section in custom_data['sections']:
            if section['callback'] == callback:
                ...
                if section.get('image'):
                    text += f"<b>Фото:</b> {section['image'][:50]}...\n"
                text += f"<b>Текст:</b> {section.get('text', '')[:100]}...\n"
                
                if section.get('buttons'):
                    text += f"\n<b>Кнопки:</b> {len(section['buttons'])}\n"
                    for btn in section['buttons']:
                        text += f"• {btn['text']} ({btn['type']})\n"
                
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🗑 Удалить", callback_data=f"custom_delete_{callback}"))
