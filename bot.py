...
def get_admin_stats():
    ...
    total_won = sum(u.get('total_won', 0) for u in users.values())
    total_spent = sum(u.get('total_spent', 0) for u in users.values())
    
    # Богатые/бедные
    rich_users = sum(1 for u in users.values() if u['coins'] >= 10000)
    poor_users = sum(1 for u in users.values() if u['coins'] < 100)
...
• Средний баланс: {avg_coins:,}
• Богатых (10k+): {rich_users}
• Бедных (<100): {poor_users}

📊 **АКТИВНОСТЬ**
• Всего сообщений: {total_messages:,}
• Среднее сообщений: {avg_messages:,}
• Всего инвайтов: {total_invites}

🎰 **ИГРЫ**
• Всего сыграно: {total_games}
• Всего выиграно: {total_won:,}
• Всего потрачено: {total_spent:,}
• Профит казино: {total_spent - total_won:,}

🎭 **РОЛИ**
• Всего куплено: {total_roles}
• Vip: {roles_stats.get('Vip', 0)}
• Pro: {roles_stats.get('Pro', 0)}
• Phoenix: {roles_stats.get('Phoenix', 0)}
• Dragon: {roles_stats.get('Dragon', 0)}
...
def callback(call):
    ...
    # Остальные обработчики игр...
        ...
        choice = parts[1]
        bet = int(parts[2])
        # Функция play_coinflip нужно добавить аналогично другим играм
        ...
        if len(data.split("_")) == 2:
            ...
            number = int(parts[1])
            bet = int(parts[2])
            # Функция play_ball нужно добавить аналогично
...
if __name__ == "__main__":
    print("🚀 Role Shop Bot с админ-статистикой запущен!")
