...
def get_full_stats():
    ...
    total_earned = sum(u.get('total_earned', 0) for u in filtered_users.values())
    ...
    active_month = sum(1 for u in filtered_users.values() if u.get('last_active', '') >= month_ago[:10])
    ...
    new_month = sum(1 for u in filtered_users.values() if u.get('registered_at', '') >= month_ago[:10])
    ...
    for role in PERMANENT_ROLES:
        owned = sum(1 for u in filtered_users.values() if role in u.get('roles', []))
        active = sum(1 for u in filtered_users.values() if role in u.get('active_roles', []))
    ...
    for user_data in daily_tasks.values():
        if user_data.get('date') == today:
            for task, data in user_data.items():
                if task in tasks_stats and data.get('completed'):
                    tasks_stats[task] += 1
    
    # Статистика по промокодам
    active_promos = sum(1 for p in promos.values() if datetime.fromisoformat(p['expires_at']) > now)
    total_promo_uses = sum(p['used'] for p in promos.values())
    total_promo_coins = sum(p['used'] * p.get('coins', 0) for p in promos.values())
    ...
    top_coins.sort(key=lambda x: x['value'], reverse=True)
    ...
    top_messages.sort(key=lambda x: x['value'], reverse=True)
    ...
    top_invites.sort(key=lambda x: x['value'], reverse=True)
    top_invites = top_invites[:10]
    ...
    return {
        ...
        'total_promo_coins': total_promo_coins,
        'top_coins': top_coins,
...
def create_backup():
    ...
    files = [USERS_FILE, PROMO_FILE, LOGS_FILE, ERRORS_FILE, ADMINS_FILE, BANS_FILE, TEMP_ROLES_FILE, ROLES_CONFIG_FILE, ECONOMY_FILE, DAILY_TASKS_FILE, TEMP_BOOST_FILE, CUSTOM_SECTIONS_FILE, STATS_FILE]
    backup_info = []
...
• Активных за неделю: <b>{stats['active_week']}</b>
• Активных за месяц: <b>{stats['active_month']}</b>
• Новых сегодня: <b>{stats['new_today']}</b>
• Новых за неделю: <b>{stats['new_week']}</b>
• Новых за месяц: <b>{stats['new_month']}</b>
...
• Всего потрачено: <b>{stats['total_spent']:,}</b>
• Всего сообщений: <b>{stats['total_messages']:,}</b>
• Всего инвайтов: <b>{stats['total_invites']}</b>

<b>Настройки экономики:</b>
• За сообщение: <b>{stats['eco']['base_reward']}</b> монет
• Бонус: <b>{stats['eco']['base_bonus_min']}-{stats['eco']['base_bonus_max']}</b> монет
• Инвайт: <b>{stats['eco']['base_invite']}</b> монет

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎭 <b>РОЛИ</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Всего куплено: <b>{stats['total_roles']}</b>
"""
    
    for role, role_stats in stats['roles_stats'].items():
        text += f"• {role}: <b>{role_stats['owned']}</b> куплено, <b>{role_stats['active']}</b> активно\n"
    
    text += f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📅 <b>ЗАДАНИЯ (сегодня)</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Написать 50 сообщений: <b>{stats['tasks_stats']['messages_50']}</b> выполнено
• Написать 100 сообщений: <b>{stats['tasks_stats']['messages_100']}</b> выполнено
• Написать 200 сообщений: <b>{stats['tasks_stats']['messages_200']}</b> выполнено
• Написать 500 сообщений: <b>{stats['tasks_stats']['messages_500']}</b> выполнено

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎁 <b>ПРОМОКОДЫ</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Всего промокодов: <b>{len(load_json(PROMO_FILE))}</b>
• Активных: <b>{stats['active_promos']}</b>
• Использовано: <b>{stats['total_promo_uses']}</b>
• Выдано монет: <b>{stats['total_promo_coins']:,}</b>

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚫 <b>БАНЫ</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
• Забанено: <b>{stats['bans_count']}</b>
...
"""
    ...
    for i, user in enumerate(stats['top_coins'], 1):
        text += f"{i}. {user['name']} — <b>{user['value']}💰</b>\n"
...
"""
    ...
    for i, user in enumerate(stats['top_messages'], 1):
        text += f"{i}. {user['name']} — <b>{user['value']}</b>\n"
    
    text += f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
👥 <b>ТОП-10 ПО ИНВАЙТАМ</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
    
    for i, user in enumerate(stats['top_invites'], 1):
        text += f"{i}. {user['name']} — <b>{user['value']}</b>\n"
...
def get_main_keyboard(page=1):
    ...
    # Кнопки навигации на одном уровне [◀️ 1/2 ▶️]
    nav_buttons = []
...
def get_shop_keyboard(page=1):
    ...
    roles = list(PERMANENT_ROLES.keys())
    per_page = 4
    ...
    # Кнопки навигации на одном уровне [◀️ 1/3 ▶️]
    nav_buttons = []
...
def get_admin_main_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("📊 Полная стата", callback_data="admin_full_stats"),
...
def get_users_navigation_keyboard(page, total_pages):
    ...
    # Кнопки навигации на одном уровне [◀️ 1/5 ▶️]
    buttons = []
    
    if page > 1:
        buttons.append(types.InlineKeyboardButton("◀️", callback_data=f"admin_users_page_{page-1}"))
    else:
...
def get_logs_navigation_keyboard(page, total_pages, log_type="logs"):
    ...
    # Кнопки навигации на одном уровне [◀️ 1/10 ▶️]
    buttons = []
    
    if page > 1:
        buttons.append(types.InlineKeyboardButton("◀️", callback_data=f"admin_{log_type}_page_{page-1}"))
    else:
        buttons.append(types.InlineKeyboardButton("◀️", callback_data="noop"))
...
def get_custom_page_navigation(section_callback, current_page, total_pages):
    ...
    if current_page > 0:
        buttons.append(types.InlineKeyboardButton("◀️", callback_data=f"custom_page_{section_callback}_{current_page-1}"))
    else:
        buttons.append(types.InlineKeyboardButton("◀️", callback_data="noop"))
    
    buttons.append(types.InlineKeyboardButton(f"{current_page+1}/{total_pages}", callback_data="noop"))
...
def show_custom_page(call, section, page_num):
    ...
    # Навигация по страницам [◀️ 1/3 ▶️]
    ...
    if image:
        try:
            bot.edit_message_media(
                types.InputMediaPhoto(image, caption=text, parse_mode='HTML'),
                call.message.chat.id,
                call.message.message_id,
                reply_markup=markup
            )
        except:
            bot.send_photo(call.message.chat.id, image, caption=text, parse_mode='HTML', reply_markup=markup)
    else:
        try:
            bot.edit_message_text(
                text,
...
def fullstats_command(message):
    if not is_master(message.from_user.id):
        return
    text = get_full_stats_text()
    bot.reply_to(message, text, parse_mode='HTML')
...
def callback_handler(call):
    ...
    if data == "back_to_main":
        ...
        try:
            ...
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=get_shop_keyboard(page))
...
▸ Ролей: <b>{len(user.get('roles', []))}</b>
        ...
        try:
            bot.edit_message_media(
                ...
                call.message.chat.id,
                call.message.message_id,
                reply_markup=get_shop_keyboard(1)
            )
        except:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=get_shop_keyboard(1))
        bot.answer_callback_query(call.id)
...
{'' if user['coins'] >= price else '❌ Не хватает монет!'}
        ...
        if data.startswith("custom_page_"):
            ...
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=get_custom_sections_keyboard())
        else:
    ...
    # Админские кнопки
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            parse_mode='HTML',
            reply_markup=get_users_navigation_keyboard(current_page, total_pages)
        )
        bot.answer_callback_query(call.id)
