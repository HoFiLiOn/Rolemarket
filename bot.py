...
def start_command(message):
    ...
    try:
        bot.send_photo(
            ...
            caption=text,
            parse_mode='HTML',
            reply_markup=get_main_keyboard(1)
        )
    except:
        bot.send_message(
            message.chat.id,
...
def callback_handler(call):
    ...
    # Навигация по магазину
        ...
        try:
            ...
            bot.send_message(call.message.chat.id, text, parse_mode='HTML', reply_markup=get_shop_keyboard(page))
        bot.answer_callback_query(call.id)
        return
    ...
    # Навигация по моим ролям
        ...
        try:
            bot.edit_message_media(
                ...
                call.message.chat.id,
                call.message.message_id,
                reply_markup=get_myroles_keyboard(roles, active, page)
            )
        except:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=get_myroles_keyboard(roles, active, page))
        bot.answer_callback_query(call.id)
        return
        ...
        if not roles:
            ...
            try:
                bot.edit_message_media(
                    ...
                    call.message.chat.id,
                    call.message.message_id,
                    reply_markup=get_myroles_keyboard(roles, active, 1)
                )
            except:
                bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=get_myroles_keyboard(roles, active, 1))
        bot.answer_callback_query(call.id)
