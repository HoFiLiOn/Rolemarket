...
def show_custom_page(call, section, page_num):
    ...
    if len(section['pages']) > 1:
        nav_buttons = []
        if page_num > 0:
            nav_buttons.append(types.InlineKeyboardButton("◀️", callback_data=f"custom_page_{section['callback']}_{page_num-1}"))
        else:
            nav_buttons.append(types.InlineKeyboardButton("◀️", callback_data="noop"))
        
        nav_buttons.append(types.InlineKeyboardButton(f"{page_num+1}/{len(section['pages'])}", callback_data="noop"))
