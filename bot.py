...
def grant_custom_title(user_id, title):
    """Выдаёт пользователю приписку (нужно быть админом чата)"""
    try:
        # Сначала проверяем, есть ли бот права администратора
        bot_member = bot.get_chat_member(CHAT_ID, bot.get_me().id)
        if bot_member.status != 'administrator':
            print("❌ Бот не является администратором чата!")
            return False
        
        # Получаем информацию о пользователе
        chat_member = bot.get_chat_member(CHAT_ID, user_id)
        
        # Если пользователь не админ - делаем его админом без прав
        if chat_member.status not in ['administrator', 'creator']:
            ...
            print(f"✅ Пользователь {user_id} назначен администратором без прав")
        
        # Устанавливаем приписку (макс 16 символов)
        custom_title = title[:16]
        bot.set_chat_administrator_custom_title(CHAT_ID, user_id, custom_title)
        print(f"✅ Приписка '{custom_title}' выдана пользователю {user_id}")
        return True
...
def handle_all_messages(message):
    ...
    # НИКАКИХ УВЕДОМЛЕНИЙ В ЛС НЕ ОТПРАВЛЯЕМ!
...
def setprice_command(message):
    """Временно меняет цену роли для теста"""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    try:
        ...
        if role_name in roles:
            roles[role_name]['price'] = new_price
            save_json(ROLES_FILE, roles)
            bot.reply_to(message, f"✅ Цена роли {role_name} изменена на {new_price}")
        else:
            bot.reply_to(message, "❌ Роль не найдена")
    except:
        bot.reply_to(message, "❌ Использование: /setprice НАЗВАНИЕ ЦЕНА")
...
if __name__ == "__main__":
    ...
    print("✅ За любое действие в чате начисляется 1 монета (без спама в ЛС)")
    print("💰 ВРЕМЕННО: все роли по 1 монете для теста")
    print("⚠️ НЕ ЗАБУДЬ сделать бота администратором чата!")
